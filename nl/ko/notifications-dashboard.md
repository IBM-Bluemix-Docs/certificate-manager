...
콜백 URL에 전송된 알림은 다음 형식으로 된 JSON 문서입니다.
...
페이로드를 디코딩 및 확인하고 나면 컨텐츠는 JSON 문자열입니다.
...
다음 샘플 코드는 알림이 전송될 때 만료되는 인증서에 대한 GitHub 문제를 작성하는 방법을 보여줍니다. {{site.data.keyword.openwhisk}}에서 이 코드를 실행하거나 다른 환경에서 코드를 사용할 수 있습니다.   
{: shortdesc}

{{site.data.keyword.openwhisk_short}}에서 이 코드를 실행하려면 다음을 수행하십시오.

1. [Cloud Functions](/docs/openwhisk/index.html#index)에서 조치를 작성하십시오.
