...
下列範例程式碼顯示在傳送通知時，如何針對到期的憑證建立 GitHub 問題。您可以在 {{site.data.keyword.openwhisk}} 執行此程式碼，或在不同環境使用程式碼。   
{: shortdesc}

若要在 {{site.data.keyword.openwhisk_short}} 執行此程式碼，請執行下列動作：

1. 在 [Cloud Functions](/docs/openwhisk/index.html#index) 建立動作。
